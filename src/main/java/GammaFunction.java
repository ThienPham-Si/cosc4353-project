...
public class GammaFunction {
    ...
    public GammaFunction() {
        ...
        // takes some time to calculate coefficients approximation for range of accuracies
        if(a > 3){
            System.out.println("");
        ...
        if (a > 0) {
            for (int k = 1; k < a; k++) {
                ...
                if ((k % 2) == 0)
                    cks[k][a] = cks[k][a].multiply(new BigDecimal(-1.0));
            }
        }
    ...
    private static BigDecimal factorial(BigDecimal number) {
        if (number.compareTo(new BigDecimal(1)) < 0)
            ...
            return number.multiply(factorial(number.subtract(new BigDecimal(1.0))));
    }
    ...
    private static BigDecimal pow_less1(BigDecimal currentResult,  BigDecimal base_cur, BigDecimal power_2, BigDecimal power, BigDecimal tolerance){
        ...
        if(base_cur.subtract(new BigDecimal(1.0)).compareTo(tolerance) < 0)
            return currentResult.multiply(base_cur);
        ...
        if(base_cur.subtract(new BigDecimal(1.0)).compareTo(tolerance) < 0)
            return currentResult.multiply(base_cur);
        if(power.subtract(power_2).compareTo(new BigDecimal("1.0E-" + DIG_ACC)) <0)
            return currentResult.multiply(base_cur);
